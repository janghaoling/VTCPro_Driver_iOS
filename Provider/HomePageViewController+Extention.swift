...
extension HomepageViewController {
    ...
    private func setValueForRatingView(){
        ...
        if userType == "COMPANY" {
            self.ratingViewNib?.labelRating.text = "\(Constants.string.rateyourtrip.localize()) \(self.requestDetail?.requests?.first?.request?.user?.company_name ?? "--")"
        } else {
    ...
    func loadAcceptNib(){ //Load Accept View XIB file
        ...
        if self.rideAcceptViewNib == nil, let rideAcceotBundle =  Bundle.main.loadNibNamed(XIB.Names.RideAcceptView, owner: self, options: nil)?.first as? RideAcceptView{
            ...
            self.rideAcceptViewNib?.btnUserProfile.addTarget(self, action: #selector(showProfilePicture(sender:)), for: .touchUpInside)
...
//            self.arrviedView?.callImage.addGestureRecognizer(gusture)
            ...
            self.arrviedView?.btnProfile.addTarget(self, action: #selector(showProfilePicture(sender:)), for: .touchUpInside)
        }
        ...
        if self.requestID != 0 {
            ...
            floaty.addItem(icon: callImage.resizeImage(newWidth: 25)) { (_) in
                ...
                DispatchQueue.global(qos: .background).async {
                    ...
                    chatObject.user_id = self.requestDetail?.requests?.first?.request?.user?.id
                ...
                if let vc = self.storyboard?.instantiateViewController(withIdentifier: Storyboard.Ids.VoiceChatViewController) as? VoiceChatViewController {
                    vc.set(connected: false, user: self.requestDetail!, requestId: User.main.id!)
                    let navigation = UINavigationController(rootViewController: vc)
