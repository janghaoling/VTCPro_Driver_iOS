...
//    if let rating = UserDefaults.standard.value(forKey: Keys.list.rating){
//        User.main.rating = rating as? Float
//    }
...
private func createCircleShapeLayer(strokeColor: UIColor, fillColor: UIColor, view: UIView, time:CFTimeInterval) -> CAShapeLayer {
...
private func createCircleShapeLayer(strokeColor: UIColor, fillColor: UIColor, view: UIView) -> CAShapeLayer {
    return createCircleShapeLayer(strokeColor: strokeColor, fillColor: fillColor, view: view, time: 60)
}

func setupCircleLayers(view: UIView, time: CFTimeInterval) ->(CAShapeLayer, CABasicAnimation){
    
    let dd = createCircleShapeLayer(strokeColor: .clear, fillColor: UIColor.primary, view: view)
    
    view.layer.addSublayer(dd)
    let animation = animatePulsatingLayer2(pulsatingLayer: dd)
    
    let trackLayer = createCircleShapeLayer(strokeColor: .secondary, fillColor: .backgroundColor, view: view, time: time) //STroke color
    view.layer.addSublayer(trackLayer)
    
    let ss: CAShapeLayer = createCircleShapeLayer(strokeColor: .clear, fillColor: .clear, view: view)
    
    ss.transform = CATransform3DMakeRotation(-CGFloat.pi / 2, 0, 0, 1)
    ss.strokeEnd = 0
    return (ss, animation)
}
...
func animatePulsatingLayer()-> CABasicAnimation {
    ...
    let time : TimeInterval = 150000
    let warn : TimeInterval = 10
...
func animatePulsatingLayer2(pulsatingLayer: CAShapeLayer)-> CABasicAnimation {
    let animation = CABasicAnimation(keyPath: "transform.scale")
