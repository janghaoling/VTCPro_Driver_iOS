...
//            present(appExistAlert, animated: true, completion: nil)
    ...
    @objc func isVoicePushRedirection() {
        ...
        if let VoicePage = self.storyboard?.instantiateViewController(withIdentifier: Storyboard.Ids.VoiceChatViewController) as? VoiceChatViewController {
            VoicePage.set(connected: true, user: self.requestDetail!, requestId: User.main.id!)
...
//            self.accountStatus = ((self.requestDetail?.account_status) ?? .none)
            if self.requestDetail?.account_status == AccountStatus.approved {
                ...
                if (self.requestDetail?.requests?.count)! > 0 {
                    ...
                    var totalAmount = requestModel?.request?.payment?.total ?? 0
                    let discount = requestModel?.request?.payment?.discount ?? 0
                    ...
                    Cache.image(forUrl: imageUrl) { (image) in
                        if image != nil {
                            DispatchQueue.main.async {
                                self.rideAcceptViewNib?.UserProfile.image = image
                            }
                        }
                    }
...
//                    self.userProfile = requestModel?.request?.user?.picture ?? ""
                    ...
                    case requestType.searching.rawValue:
                        ...
                        if self.isScheduled, let dateObject = Formatter.shared.getDate(from: requestModel?.request?.schedule_at, format: DateFormat.list.yyyy_mm_dd_HH_MM_ss) {  // Set Date For  Scheduled Request
                            ...
                            self.rideAcceptViewNib?.displaySchduleView(isScheduled: true)
                        }
                        else {
//                            print("noschedule")
                            self.rideAcceptViewNib?.displaySchduleView(isScheduled: false)
//                            self.view.setNeedsLayout()
                        }
                        
                        self.startTimer(requestModel?.time_left_to_respond)
                    ...
                    case requestType.pickedUp.rawValue:
                        ...
                        if let way_points = requestModel?.request?.way_points {
                            ...
                            let stopLat = wayPnts?.first?.lat
                            let stopLng = wayPnts?.first?.lng

                            //MARK:- set Marker for dource and destination
    //                        if destinationLatitde != 0 && destinationLontitude != 0 {
                            self.setMarker(sourceLat: sourceLatitute, sourceLong: sourceLongtitude, destinationLat: stopLat ?? 0, destinationLong: stopLng ?? 0)
                                
                            self.drawPoly(s_latitude: sourceLatitute, s_longtitude: sourceLongtitude, d_latitude: stopLat ?? 0, d_longtitude: stopLng ?? 0)
                                
    //                        }
                            self.setValueForGoogleRetractionView(status: requestType.midstopped.rawValue)
                        } else {
    ...
    //                        }
                    ...
                    case requestType.midstopped.rawValue:
                        print("*****************")
                        self.viewGoogleRetraction.showAnimateView(self.viewGoogleRetraction, isShow: true, direction: .Top)
                        self.loader.isHidden = true
                        self.gMSmapView.clear()
                        
                        let way_points = requestModel?.request?.way_points
                        let wayPnts:[WayPoint]? = way_points?.data(using: .utf8)?.getDecodedObject(from: [WayPoint].self)
                        let stopLat = wayPnts?.first?.lat
                        let stopLng = wayPnts?.first?.lng
                        ...
                        self.loadAndShowArrivedNib()
                        self.timer = Timer.scheduledTimer(timeInterval: 0.2, target: self, selector: #selector(HomepageViewController.startPickupToDropProgess), userInfo: nil, repeats: true)
                        self.setTitleForButton(status: requestType.midstopped.rawValue)
                        self.arrviedView?.imagePickup.image = UIImage(named: "pickup-select")
                        
                        self.setValueForGoogleRetractionView(status: requestType.dropped.rawValue)
                        self.floatyButton?.removeFromSuperview()
                        
                        self.setMarker(sourceLat: destinationLatitde, sourceLong: destinationLontitude, destinationLat: stopLat ?? 0, destinationLong: stopLng ?? 0)

                        self.drawPoly(s_latitude: destinationLatitde, s_longtitude: destinationLontitude, d_latitude: stopLat ?? 0, d_longtitude: stopLng ?? 0)
                        
//                        self.setMarker(sourceLat: stopLat ?? 0, sourceLong: stopLng ?? 0, destinationLat: destinationLatitde, destinationLong: destinationLontitude)
//
//                        self.drawPoly(s_latitude: stopLat ?? 0, s_longtitude: stopLng ?? 0, d_latitude: destinationLatitde, d_longtitude: destinationLontitude)
                        self.getUserCurrentLocation()//MARK:- Here Update the current location and set "markerProviderLocation" marker.Once it will set it will update the current location.
                    ...
                    case requestType.dropped.rawValue:
                        ...
                        DispatchQueue.main.async {
                            ...
                            self.setValueForInvoiceView(bookingId: bookingId, total: totalAmount, paymentMode: paymentMode, payable: payable)
                        }
                    ...
                    let userType = requestModel?.request?.user?.user_type
                    if userType == "COMPANY" {
                        self.rideAcceptViewNib?.userName.text = requestModel?.request?.user?.company_name
                    }
                    else {
                    ...
                    // it there is stop location then show else hide
                    if let way_points = self.requestDetail?.requests?.first?.request?.way_points {
                        let wayPnts:[WayPoint]? = way_points.data(using: .utf8)?.getDecodedObject(from: [WayPoint].self)
                        let stopAddress = wayPnts?.first?.address
                        self.rideAcceptViewNib?.labelDropLocationValue.text = stopAddress ?? ""
                        self.rideAcceptViewNib?.LabelStopValue.text = requestModel?.request?.d_address
                    } else {
                        self.rideAcceptViewNib?.LabelStopValue.isHidden = true
                        self.rideAcceptViewNib?.LabelStop.isHidden = true
                    }
                    
                    if let note = self.requestDetail?.requests?.first?.request?.note, note.count > 0 {
                        self.rideAcceptViewNib?.labelNoteValue.text = note
                    } else {
                        self.rideAcceptViewNib?.labelNoteValue.isHidden = true
                        self.rideAcceptViewNib?.labelNote.isHidden = true
                    }
                    
                    self.rideAcceptViewNib?.ivPayMethod.image = self.requestDetail?.requests?.first?.request?.payment_mode == PaymentType.CARD.rawValue ? UIImage(named: "visa") : UIImage(named: "money")
                    self.rideAcceptViewNib?.labelPrice.text = Formatter.shared.appPriceFormat(string: "\(requestModel?.request?.total_price ?? 0)")
                    
                    if let pool_commission = requestModel?.request?.pool_commission {
                        let price = (100 - (pool_commission as NSString).floatValue) * (requestModel?.request?.total_price ?? 0.0) / 100
                        self.rideAcceptViewNib?.labelPrice.text = Formatter.shared.appPriceFormat(string:"\(price)")
                    }

                    
                    self.rideAcceptViewNib?.lbPayMethod.text = self.requestDetail?.requests?.first?.request?.payment_mode?.lowercased().localize()
...
//        }
    ...
    func plotDestinationMarker(coordinate : CLLocationCoordinate2D) {
        self.destinationMarker.position = coordinate
...
//        let currency = UserDefaults.standard.value(forKey: Keys.list.currency)
        ...
        if requestDetail?.requests?.first?.request?.use_wallet == 1 && self.requestDetail?.requests?.first?.request?.user?.user_type == "COMPANY" {
            
            payMode = "WALLET"
        }
        
        self.inVoiceView?.labelCash.text = payMode
//        self.inVoiceView?.labeltotalValue.text = "\(currency ?? "$") \(total ?? 0)"
        self.inVoiceView?.labeltotalValue.text = Formatter.shared.appPriceFormat(string: "\(total ?? 0)")
//        self.inVoiceView?.labelAmountTobePaid.text = "\(currency ?? "$") \((payable ?? 0))"
        self.inVoiceView?.labelAmountTobePaid.text = Formatter.shared.appPriceFormat(string: "\(payable ?? 0)")
    ...
    func setValueForArrivedView(){
        let userType = self.requestDetail?.requests?.first?.request?.user?.user_type
        if userType == "COMPANY" {
            self.arrviedView?.userName.text = self.requestDetail?.requests?.first?.request?.user?.company_name
        }
        else {
...
//        })
    ...
    private func  setValueForGoogleRetractionView(status: String?){
        if status == requestType.started.rawValue {
            ...
            self.labelPickUp.text = Constants.string.stopLocation.localize()
            if let way_points = self.requestDetail?.requests?.first?.request?.way_points {
                let wayPnts:[WayPoint]? = way_points.data(using: .utf8)?.getDecodedObject(from: [WayPoint].self)
                let stopAddress = wayPnts?.first?.address
//                self.labelPickupValue.text = stopAddress ?? ""
                self.labelPickupValue.text = self.requestDetail?.requests?.first?.request?.d_address
            } else {
                self.labelPickupValue.text = ""
            }
        } else if status == requestType.dropped.rawValue {
            self.labelPickUp.text = Constants.string.dropLocation.localize()
            if let way_points = self.requestDetail?.requests?.first?.request?.way_points {
                let wayPnts:[WayPoint]? = way_points.data(using: .utf8)?.getDecodedObject(from: [WayPoint].self)
                let stopAddress = wayPnts?.first?.address
...
//        vibrate(sound: .pop)
    ...
    func startTimer(_ time : Int?){ //MARK:- Here set the timer value for accept request counter
    ...
    @objc func showProfilePicture(sender: UIButton) {
        let vc = FullScreenSlideshowViewController()
        vc.inputs = [
            AlamofireSource(urlString: self.iurl ?? "")!
        ]
...
//        }
    ...
    @objc  func arrivedButtontapped(sender: UIButton){
        ...
        let requestModel:RequestInsideModel? = self.requestDetail?.requests![0]
        let requestStatus = requestModel?.request?.status
        ...
        if requestStatus == requestType.arrived.rawValue {
            ...
            if let _ = requestModel?.request?.way_points { // arrived stop location
                self.statusChanged(status: requestType.midstopped.rawValue, tollFee: 0)
            } else { // dropped
                showTollView()
            }
        }
        else if requestStatus == requestType.midstopped.rawValue { // dropped
            showTollView()
        }
        else { // arrived
            self.loader.isHidden = false
    ...
    func showTollView() {
        ...
        self.tollView?.onClickDismiss = { tollFee, isDismiss in
            ...
            if isDismiss {
                ...
                let toll = tollFee.replacingOccurrences(of: ",", with: ".")
                self.statusChanged(status: requestType.dropped.rawValue, tollFee: Double(toll)!)
            }
    ...
    func statusChanged(status : String,tollFee:Double){ //MARK:- changing the button name occurding to the request status
        ...
        case requestType.pickedUp.rawValue:
            ...
            if let _ = self.requestDetail?.requests?.first?.request?.way_points {
                self.arrviedView?.arrivedBtn.setTitle(Constants.string.tapWhenStopped.localize().uppercased(), for: .normal)
            ...
            } else {                self.arrviedView?.arrivedBtn.setTitle(Constants.string.tapWhenDropped.localize().uppercased(), for: .normal)
                self.arrviedView?.cancelBtn.alpha = 1.0
                self.arrviedView?.hideWaitingTime = false
                UIView.animate(withDuration: 0.5, animations: {
                    self.arrviedView?.cancelBtn.alpha = 0.0
        ...
        case requestType.dropped.rawValue:
            ...
            self.arrviedView?.hideWaitingTime = true
        case requestType.midstopped.rawValue:
            self.LoadUpdateStatusAPI(status: status, tollFee: 0)
            self.arrviedView?.arrivedBtn.setTitle(Constants.string.tapWhenDropped.localize().uppercased(), for: .normal)
            self.arrviedView?.cancelBtn.alpha = 1.0
            self.arrviedView?.hideWaitingTime = false
        default:
            print("default")
