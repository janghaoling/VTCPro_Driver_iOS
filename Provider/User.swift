...
class User : NSObject, NSCoding, JSONSerializable {
    ...
    required convenience init?(coder aDecoder: NSCoder) {
        ...
        let rating = aDecoder.decodeObject(forKey: Keys.list.rating) as? String
        let service_image = aDecoder.decodeObject(forKey: Keys.list.service_image) as? String
        
        
            self.init(id: id, name: name, accessToken: accessToken, latitude: latitude, lontitude: lontitude, firstName: firstNmae, lastName: lastName, email: email, phoneNumber: phoneNumber, country_code: country_code, serviceType: serviceType, serviceId : serviceId, picture: picture, currency: currency, walletBalance : walletBalance, measurement : measurement, isCardAllowed : isCardAllowed, stripeKey : stripeKey,login_by:login_by,referral_count:referral_count,referral_unique_id:referral_unique_id,referral_total_text:referral_total_text,referral_text:referral_text,otp:otp,ride_otp:ride_otp,service_model:service_model,service_number:service_number, usertype:usertype,rating:rating,service_image:service_image)
    }
    ...
    func encode(with aCoder: NSCoder) {
        ...
        aCoder.encode(self.rating , forKey: Keys.list.rating)
        aCoder.encode(self.service_image , forKey: Keys.list.service_image)
