...
class SignUpTableViewController: UITableViewController, UIGestureRecognizerDelegate {
    ...
    var terms_conditions_url = "https://memohi.fr/terms"
    //MARK:- Variable
...
//    let accountKit = AKFAccountKit(responseType: .accessToken)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        Webservice().retrieve(api: .terms_conditions, url: nil, data:nil, imageData: nil, paramters: nil, type: .GET) { (error, data) in
            ...
            self.terms_conditions_url = responseObject?.url ?? "https://memohi.fr/terms"
        }
...
extension SignUpTableViewController {
    override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        case 0...8:
            if indexPath.row == 8, self.viewReferCode.isHidden {
                return 0
            }
            return 80
        case 9:
...
//        self.present(accountKitVC, animated: true, completion: nil)
    ...
    private func getSMS() {
        if let getSMSVC = Router.user.instantiateViewController(withIdentifier: Storyboard.Ids.GetSMSViewController) as? GetSMSViewController {
            getSMSVC.phone = textFieldPhoneNumber.text!
            getSMSVC.email = EmailTxt.text!
            getSMSVC.country_Code = countryCode ?? "FR"
            getSMSVC.onCompleteVerified = { code, phone in
                self.navigationController?.popToViewController(self, animated: true)
                self.textFieldPhoneNumber.text = phone
