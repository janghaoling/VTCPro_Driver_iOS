...
extension SideBarTableViewController {
    ...
    private func setValues(){
        ...
        self.lbWallet.text = "\(Formatter.shared.appPriceFormat(string: "\(Float.removeNil(User.main.walletBalance))"))"
        self.lbWallet.textColor = Float.removeNil(User.main.walletBalance) < 0 ? .red : .black
    }
...
extension SideBarTableViewController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.row == 0 {//yourtrips
            ...
            let currRideVC = Router.main.instantiateViewController(withIdentifier: Storyboard.Ids.CurrentRideViewController)
            baseVC?.pushViewController(currRideVC, animated: true)
        } else {
            self.logout()
        }
        self.drawerController?.closeSide()
