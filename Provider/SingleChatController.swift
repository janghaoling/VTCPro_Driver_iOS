...
extension SingleChatController {
    ...
    func set(user : GetRequestModelResponse, chatType : ChatType = .single,requestId : Int){
        
        self.currentUser = user.requests?.first?.request?.user
    ...
    private func updateUI() {
        ...
        if currentUser?.user_type! != nil {
            let userType = String.removeNil(currentUser.user_type)
            if userType == "COMPANY" {
                self.navigationItem.title = String.removeNil(currentUser.company_name)
            } else {
...
//        self.navigationItem.title = String.removeNil(currentUser.first_name)+" "+String.removeNil(currentUser.last_name)
        if currentUser?.picture! != nil {
            Cache.image(forUrl: Common.getImageUrl(for: currentUser.picture)) { (image) in
                if image != nil {
                    DispatchQueue.main.async {
    ...
    private func initialLoads(){
        ...
        let backButtonArrow = UIBarButtonItem(image: #imageLiteral(resourceName: "back-icon").resizeImage(newWidth: 30)?.withRenderingMode(.alwaysOriginal) , style: .plain, target: self, action: #selector(self.backAction))
        let fixedSpace = UIBarButtonItem(barButtonSystemItem: .fixedSpace, target: self, action: nil)
        let imageButton = UIBarButtonItem(image: #imageLiteral(resourceName: "young-male-avatar-image").resizeImage(newWidth: 30)?.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(self.navigationBarTapped))
        if currentUser?.picture! != nil {
            Cache.image(forUrl: Common.getImageUrl(for: currentUser.picture)) { (image) in
                if image != nil {
                    DispatchQueue.main.async {
    ...
    private func getImagePreview(index : Int)->LightboxController?{
        ...
        if datasource.count>index, datasource[index].response?.type == Mime.image.rawValue, let url = URL(string: (datasource[index].response?.url) ?? .Empty) {
            ...
            LightboxConfig.CloseButton.image = #imageLiteral(resourceName: "close").resizeImage(newWidth: 30)
...
extension SingleChatController {
    ...
    @objc private func cameraOnclick(){
    ...
    //Record Onclick Action
    @objc private func sendOnclick(){
        
        FirebaseHelper.shared.write(to: self.requestId, with: self.textViewSingleChat.text, type : self.chatType, userId : currentUserId, driverId : Int.removeNil(User.main.id))
...
extension SingleChatController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let chat = datasource[indexPath.row].response, let tableCell = tableView.dequeueReusableCell(withIdentifier: getCellId(from: chat), for: indexPath) as? ChatCell {
            ...
            if chat.sender == UserType.provider.rawValue {
    ...
    private func getCellId(from entity : ChatEntity)->String {
        ...
        if entity.sender == UserType.provider.rawValue {
