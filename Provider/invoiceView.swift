...
//        self.labelAmountTobePaid.text = "\(User.main.currency ?? "$") \(Formatter.shared.limit(string: "\(request.payment?.payable ?? 0)", maximumDecimal: 2))"
        ...
        print("&&&&&&&&&&&&&&", request.payment?.discount ?? 0)
        if discount > 0 {
            self.labeltotalValue.text = Formatter.shared.appPriceFormat(string: "\(request.payment?.total ?? 0 + discount)")
        } else {
            self.labeltotalValue.text = Formatter.shared.appPriceFormat(string: "\(request.payment?.total ?? 0)")
        }
        
        self.labelAmountTobePaid.text = Formatter.shared.appPriceFormat(string: "\(request.payment?.payable ?? 0)")
        ...
        self.labelTips.text = Formatter.shared.appPriceFormat(string: "\(request.payment?.tips ?? 0)")
