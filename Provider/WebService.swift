...
class Webservice : PostWebServiceProtocol {
    ...
    private func send(api: Base, imageData: [String:Data]?, parameters: [String : Any]?){
        ...
        headers.update(name: api.rawValue, value: WebConstants.string.secretKey)

       // headers.updateValue(WebConstants.string.multipartFormData, forKey: WebConstants.string.Content_Type)

        headers.update(name: WebConstants.string.XMLHttpRequest, value: WebConstants.string.X_Requested_With)

        if api != .login {
            if let accesstoken = User.main.accessToken {
                
                headers.update(name: "Bearer \(accesstoken)", value: WebConstants.string.Authorization)
                print("Bearer \(accesstoken)")
